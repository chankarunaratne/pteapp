...
export default function McmaSessionPage() {
  ...
  const question = MCMA_QUESTIONS[index];
  ...
  const handleNext = () => {
    if (index + 1 >= MCMA_QUESTIONS.length) {
      setPhase("summary");
    } else {
      setIndex(index + 1);
      setSelectedIndices([]);
  ...
  return (
    <div>
      <div className="text-sm font-normal text-slate-400 flex items-center">
        ...
        <span className="mx-1.5">/</span>
        <span className="text-slate-600 font-medium">Multiple-choice, Choose Multiple Answers</span>
      </div>
      <div className="mt-4 flex items-center justify-between">
        <h1 className="text-2xl font-bold tracking-tight">
          Question {index + 1} of {MCMA_QUESTIONS.length}
        </h1>
      </div>
      ...
      <div className="mt-4 h-1.5 w-full overflow-hidden rounded-full bg-slate-200">
        <div
          ...
          style={{
            width: `${((index + (phase === "result" ? 1 : 0)) / MCMA_QUESTIONS.length) * 100}%`,
          }}
        />
      </div>
      ...
      <div className="mt-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="text-base font-medium text-slate-700 flex items-start gap-1">
          <span>
            Listen to the recording and answer the question by selecting the correct responses. You will need to select more than one response.
          </span>
          <InfoPopover align="right">
            පටිගත කිරීමට සවන් දී නිවැරදි පිළිතුරු තෝරා ප්‍රශ්නයට පිළිතුරු සපයන්න. ඔබට එකකට වඩා වැඩි පිළිතුරු ප්‍රමාණයක් තෝරා ගැනීමට සිදුවේ.
          </InfoPopover>
        </div>
                  ...
                  <div className="flex items-start gap-3">
                    <div className="flex h-5 items-center">
                      <input
                        ...
                        checked={isSelected}
                        disabled
                        className="h-4 w-4 rounded border-slate-300 text-brand-600 focus:ring-brand-500 disabled:opacity-75"
                      />
                    </div>

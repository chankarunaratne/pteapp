...
import McsaScoreOverview from "@/components/mcsa/McsaScoreOverview";
import McsaFeedbackPanel from "@/components/mcsa/McsaFeedbackPanel";
import { useLanguage } from "@/lib/LanguageContext";
import { MCSA_QUESTIONS } from "@/lib/questions";
import { scoreMcsaAnswer, type McsaScoreResult } from "@/lib/mcsaScoring";

type Phase = "question" | "result" | "summary";

export default function McsaSessionPage() {
  const [phase, setPhase] = useState<Phase>("question");
  const [index, setIndex] = useState(0);
  const [selectedIndex, setSelectedIndex] = useState<number | null>(null);
  const [scores, setScores] = useState<McsaScoreResult[]>([]);
  const { lang } = useLanguage();
  const [feedbackLoading, setFeedbackLoading] = useState(false);
  const [restartCount, setRestartCount] = useState(0);
  const feedbackTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);

  const question = MCSA_QUESTIONS[index];
  ...
  const handleSubmit = () => {
    ...
    setScores((prev) => [...prev, scoreMcsaAnswer(question, selectedIndex)]);
  ...
  const handleNext = () => {
    if (index + 1 >= MCSA_QUESTIONS.length) {
      setPhase("summary");
    } else {
      setIndex(index + 1);
      setSelectedIndex(null);
  ...
  return (
    <div>
      <div className="text-sm font-normal text-slate-400 flex items-center">
        ...
        <span className="mx-1.5">/</span>
        <span className="text-slate-600 font-medium">Multiple-choice, Choose Single Answer</span>
      </div>
      <div className="mt-4 flex items-center justify-between">
        <h1 className="text-2xl font-bold tracking-tight">
          Question {index + 1} of {MCSA_QUESTIONS.length}
        </h1>
      </div>
      ...
      <div className="mt-4 h-1.5 w-full overflow-hidden rounded-full bg-slate-200">
        <div
          ...
          style={{
            width: `${((index + (phase === "result" ? 1 : 0)) / MCSA_QUESTIONS.length) * 100}%`,
          }}
        />
      </div>
      ...
      <div className="mt-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="text-base font-medium text-slate-700 flex items-start gap-1">
          <span>
            Listen to the recording and answer the multiple-choice question by selecting the correct response.
          </span>
          <InfoPopover align="right">
            පටිගත කිරීමට සවන් දී නිවැරදි එක් පිළිතුරක් පමණක් තෝරා ප්‍රශ්නයට පිළිතුරු සපයන්න.
          </InfoPopover>
        </div>
          ...
          {question.options.map((option, oIdx) => {
            ...
            if (phase === "question") {
              const optionClasses = isSelected
                ? "bg-yellow-100/70 border-yellow-400 text-slate-800 shadow-sm"
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-start gap-3 rounded-xl border p-4 text-left transition duration-200 cursor-pointer focus:outline-none focus:ring-2 focus:ring-brand-100 ${optionClasses}`}
                >
                  <div className="flex h-5 items-center">
                    <input
                      ...
                      checked={isSelected}
                      readOnly
                      className="h-4 w-4 border-slate-300 text-brand-600 focus:ring-brand-500 cursor-pointer"
                    />
                  </div>
                  ...
                  <div className="flex items-start gap-3">
                    <div className="flex h-5 items-center">
                      <input
                        ...
                        checked={isSelected}
                        disabled
                        className="h-4 w-4 border-slate-300 text-brand-600 focus:ring-brand-500 disabled:opacity-75"
                      />
                    </div>
          ...
          <div className="mt-4 rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
            {feedbackLoading ? (
              ...
              <div className="animate-feedback-reveal p-6">
                ...
                <div className="mt-4">
                  <McsaScoreOverview score={currentScore} lang={lang} />
                </div>
  ...
  const missedOptions = [
    ...new Set(
      scores.flatMap((s, idx) => {
        const question = MCSA_QUESTIONS[idx];
    ...
    <div className="animate-feedback-reveal">
      ...
      <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-3">
        <div className="rounded-2xl border border-slate-200 bg-white p-5 text-center shadow-sm">
          ...
          <p className="mt-1 text-sm text-slate-500">Total Points</p>
        </div>
        ...
        {missedOptions.length > 0 ? (
          <div className="mt-3 flex flex-col gap-2">
            {missedOptions.map((option) => (
              <div
                key={option}
                className="rounded-xl border border-slate-100 bg-slate-50/50 p-3 text-sm font-medium text-slate-700"
              >
                {option}
              </div>
            ))}

...
export default function RmcsaSessionPage() {
  ...
  const question = RMCSA_QUESTIONS[index];
  ...
  const handleNext = () => {
    if (index + 1 >= RMCSA_QUESTIONS.length) {
      setPhase("summary");
    } else {
      setIndex(index + 1);
      setSelectedIndex(null);
    ...
    <div>
      ...
      <div className="mt-4 flex items-center justify-between">
        <h1 className="text-2xl font-bold tracking-tight">
          Question {index + 1} of {RMCSA_QUESTIONS.length}
        </h1>
      </div>
      ...
      <div className="mt-4 h-1.5 w-full overflow-hidden rounded-full bg-gray-200">
        <div
          ...
          style={{
            width: `${((index + (phase === "result" ? 1 : 0)) / RMCSA_QUESTIONS.length) * 100}%`,
          }}
        />
      </div>
      ...
      <div className="mt-6 rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
        ...
        <div className="text-base font-medium text-gray-700 flex items-start gap-1">
          ...
          <InfoPopover align="right">
            පෙළ කියවා නිවැරදි එක් පිළිතුරක් පමණක් තෝරා ප්‍රශ්නයට පිළිතුරු සපයන්න.
          </InfoPopover>
        </div>
        ...
        <>
          ...
          {!feedbackLoading && (
            <div className="mt-4 flex justify-end">
              ...
              >
                {index + 1 >= RMCSA_QUESTIONS.length
                  ? "See Summary"
                  : "Next Question"}
              </button>
            </div>
  ...
  const missedOptions = [
    ...new Set(
      scores.flatMap((s, idx) => {
        const question = RMCSA_QUESTIONS[idx];

...
import type { HcsScoreResult } from "@/lib/hcsScoring";
import type { FeedbackLang } from "@/lib/feedback";

export default function HcsFeedbackPanel({
  score,
  explanationEn,
  explanationSi,
  lang = "si",
}: {
  score: HcsScoreResult;
  ...
  if (lang === "si") {
    if (score.isCorrect) {
      headline = "විශිෂ්ටයි! නිවැරදි සාරාංශය තෝරාගෙන තිබේ.";
    ...
    tips.push("ඡේදය පුරාම දිවෙන ප්‍රධාන අදහස (main idea) තේරුම් ගැනීමට උත්සාහ කරන්න; තනි තනි වචන හෝ කුඩා විස්තර වලට පමණක් රැවටෙන්න එපා.");
    tips.push("PTE Highlight Correct Summary ප්‍රශ්න වලදී වැරදි පිළිතුරු සඳහා සෘණ ලකුණු (negative marks) ලැබෙන්නේ නැත. එබැවීම් සැමවිටම පිළිතුරක් තෝරන්න.");
  } else {
    if (score.isCorrect) {
      headline = "Excellent! You selected the correct summary.";
      tips.push("Keep focusing like this on the next question.");
    } else {
      headline = "Don't worry, let's learn from this attempt.";
      tips.push("Review the correct summary and the explanation below.");
    }
    tips.push("Focus on the overall main idea of the recording rather than isolated words or minor details.");
    tips.push("For HCS questions, there is no negative penalty for incorrect choices. Make sure to always guess if you are unsure.");
  }
